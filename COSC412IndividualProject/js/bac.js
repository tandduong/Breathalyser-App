...
var METABOLIC_REMOVAL_RATE_GPH = 7.9;		//  g/hr
...
$('#add').on('click', function () {
  ...
  if (incompleteBio) {
    return alert('Please fill all the neccessary information...');
  }
  ...
  var icon = 'hairy';
  if (outputBAC > 0.35) {
    status = 'You better not to drive!!';
    icon = 'cuddly';
  } else if (outputBAC > 0.26) {
    status = 'You need to rest before driving';
    icon = 'cuddly';
  } else if (outputBAC > 0.19) {
    status = 'I bet you are still drunk. Do not take your key right now.';
    icon = 'devil';
  } else if (outputBAC > 0.12) {
    status = 'You better rest a bit please.';
    icon = 'devil';
  } else if (outputBAC > 0.07) {
    status = 'You are OK to take a trip home';
    icon = 'gummy';
  } else if (outputBAC > 0.05) {
    status = 'You can, safe drive!';
    icon = 'gummy';
  } else if (outputBAC > 0.035) {
    status = 'Just go home, you are good';
    icon = 'hairy';
  ...
  var $info = $('<div class="post"><p>Amount of drink: <strong>' + outputIngestedMl +
    ' ml</strong><br>Remain inside body: <strong>' + outputRemainingMl +
    ' ml</strong><br>BAC: <strong>' + outputBAC +
    ' %</strong><br>Time to BAC back to 0: <strong>' + minutesToOhFive +
    ' mins</strong><br>Your advice: <strong>' + status +
    ' </strong></p></div>');
  $('.info').empty();
